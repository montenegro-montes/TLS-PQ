...
def process_file(input_file_path, output_file_path):
    ...
    # Line that marks the start of a new group
    pattern_new_group = re.compile(r"^Certificate request self-signature ok")
    ...
    for line in lines:
        ...
        # Check for a new group marker
        if pattern_new_group.match(line):
            if primero:
              primero = False #La primera ejecucion está vacía, pero puede encontrarse alguna mas que no haya ejecutado
            else:
              #if connections_group:  # Save the current group if it exists
              connections.append(connections_group)
              connections_group = []  # Start a new group
              continue    

    # Add any remaining connections group
