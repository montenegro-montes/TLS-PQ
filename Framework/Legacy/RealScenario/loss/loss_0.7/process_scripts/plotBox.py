...
for i, signature in enumerate(signature_algorithms):
    ...
    for j, kem in enumerate(kems):
        ...
        kem_data_filtered = kem_data[~np.isnan(kem_data)]

        if len(kem_data_filtered) > 0:  # Asegurar que haya datos
            # Calcular estadísticos para la barra
            mean = np.mean(kem_data_filtered)
            std = np.std(kem_data_filtered)
            ...
            # Dibujar el boxplot
            bp = ax.boxplot(kem_data_filtered, positions=[current_position], widths=bar_width * 0.8,
                            patch_artist=True,
