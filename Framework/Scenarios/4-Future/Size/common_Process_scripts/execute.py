...
def run_script_and_process(script_name, log_file, csv_file, logs_dir, output_dir):
    ...
    print(f"Ejecutando script: {script_name}.sh")
...
parser.add_argument('--tag', required=True, help='Sufix to add to generated files, i.e. "_1ms".')
...
print("Moving files  .log to folder  'logs'...")
for log in logs:
...
for log, csv_output in zip(logs, csvs):
    ...
    if os.path.isdir(csv_output):
        ...
        if os.path.exists(generated_file):
            ...
            raise FileNotFoundError(f"Not found generated file inside of {csv_output}.")
