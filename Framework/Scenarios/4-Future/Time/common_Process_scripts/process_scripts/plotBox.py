...
df.columns = [col.replace('CROSSrsdpg128balanced', 'cross128') for col in df.columns]
df.columns = [col.replace('CROSSrsdpg192balanced', 'cross192') for col in df.columns]
df.columns = [col.replace('CROSSrsdpg256balanced', 'cross256') for col in df.columns]
...
bar_width = 0.5  # Reducir ancho de barras para que todas estén más juntas dentro del grupo
...
ax.set_xlabel('Signature Algorithm', fontsize=26,fontweight='bold', labelpad=15)
ax.set_ylabel('Number of Handshakes', fontsize=26, fontweight='bold')
ax.set_xticks(group_positions)

signature_wrapped = [
    signature.replace('_', '\n', 1) if '_' in signature else signature
    for signature in signature_algorithms
]

ax.set_xticklabels(signature_wrapped , fontsize=22)
...
ax.legend(handles, kems, title="KEM Algorithm", fontsize=22, title_fontsize=24, loc='best')
